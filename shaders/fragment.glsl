 // Fragment shader
uniform sampler2D uImage;  // The texture uniform
varying vec2 vUv;
uniform float uTime 
...
{
    if(index == 0) {return texture2D(uImage1,uv)}
    else if (index ==1){ return texture2D(uImage2,uv)
}
 else if (index == 2){ return texture2D(uImage3,uv)
}
 else if (index == 3){ return texture2D(uImage4,uv)
}
...
void main() {
    ...
    float strengt = smoothstep(.0 ,1.0,uTimeline) - smoothstep(2.0,3.0,uTimeline)

    float distort =  mix(1.0,1.2+strengt,wave)
    uv += .5
    uv *= distort   
    ...
    gl_FragColor = tex;  // Output the color from the texture
}
